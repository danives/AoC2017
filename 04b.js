...
function process(a, index) {
    for (var w in words) {
        var alpha_word_c = words[w].split("").sort().join("");
        if (w != index && alpha_word_c === a) {
            return false;
        }
    }
    return true;
...
for (var i in items) {
    ...
    for (var word in words) {
        var alpha_word = words[word].split("").sort().join("");
        var result = process(alpha_word, word);
        if (!result) {
            valid = false;
            break;
        }
