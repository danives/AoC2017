var input = `3751   3769    2769    2039    2794    240 3579    1228    4291    220 324 3960    211 1346    237 1586
...
function process(a) {
    for (var num in numbers) {
        if (numbers[num].trim() != a && numbers[num].trim() != '') {
            var comp = parseInt(numbers[num].trim());
            if (a % comp === 0) {
                return a/comp;
            }
        }
...
for (var i in items) {
    var found = 0;
    var numbers = items[i].split(" ");
    for (var j in numbers) {
        if (numbers[j].trim() != '') {
            var comparison = parseInt(numbers[j].trim());
            var result = process(comparison);
            if (result !== false) {
                found = result;
                break;
            }
