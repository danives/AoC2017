...
// Extract the variables to the window
for (var i in processed) {
    var v = (processed[i]+"").split(" ");
    if (v.length > 4) {
        window[v[0]] = 0;
        variables.push(v[0]);
        window[v[4]] = 0;
...
for (var i in processed) {
    var eq = processed[i].split("if");
    var valid = eval(eq[1]);
    if (valid) {
        var parts = processed[i].split(" ");
        window[parts[0]] = eval(eq[0].replace("dec", "-").replace("inc", "+"));
    }
}

// Now determine the variable with the highest value
var h_variable = false;
var highest = 0;
for (var i in variables) {
    if (window[variables[i]] > highest) {
        highest = window[variables[i]];
        h_variable = variables[i];
    }
}

console.log(h_variable);
console.log(highest);