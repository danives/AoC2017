...
for (var i in split) {
    var entry = split[i];
    var branch = entry.substring(0, entry.indexOf("(")-1).trim();
    var index = entry.indexOf("->");
    var lbracket = entry.indexOf("(");
    var rbracket = entry.indexOf(")");
    var item_weight = parseInt(entry.substring(lbracket+1, rbracket).trim());
    if (index == -1) {    
        dead_branches[branch] = { name: branch, weight : item_weight };
    } else {
        roots[branch] = { name: branch, weight : item_weight, branch_names : entry.substring(index+3, entry.length).split(", ") };
    }
}
...
for (var r in roots) {
    found = false;
    for (var c in roots) {
        if (roots[c].branch_names.indexOf(r) > -1) {
            found = true;
            break;
        }
    }
...
// Now we have the base, lets start building the tree
function process(item)
{
    var branches = {};
    for (var bn in item.branch_names) {
        var name = item.branch_names[bn];
        if (dead_branches.hasOwnProperty(name)) {
            branches[name] = dead_branches[name];
        } else if (roots.hasOwnProperty(name)) {
            branches[name] = process(roots[name]);
        }
    }
    
    var weights = {};
    var weight_counts = {};
    item.weight_above = item.weight;
    
    // Get all the weights above the branch
    for (b in branches) {
        if ("weight_above" in branches[b]) {
            weights[b] = branches[b].weight_above;
        } else {
            weights[b] = branches[b].weight;
        }
        item.weight_above += weights[b];
        
        if (weights[b] in weight_counts) {
            weight_counts[weights[b]].push(b);
        } else {
            weight_counts[weights[b]] = [b];
        }
    }

    item.branches = branches;

    // If we have a difference in weights
    if (Object.keys(weight_counts).length > 1 && !problem_found) {
        problem_found = true;
        var problem_branch;
        var correct_weight, incorrect_weight;
        
        // We found the branch that contains a baddie
        for (i in weight_counts) {
            if (weight_counts[i].length > 1) {
                // This is the comparison weight
                correct_weight = i;
            } else {
                incorrect_weight = i;
                // So we need to adjust the weight of this branch
                problem_branch = item.branches[weight_counts[i][0]];
            }
        }
        console.log(item);
        console.log(problem_branch.weight);
        console.log(problem_branch.weight - (incorrect_weight - correct_weight));
    }

    return item;
}

var tree = process(tree[r]);
